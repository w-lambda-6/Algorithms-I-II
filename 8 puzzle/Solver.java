...
public class Solver {
    ...
    private SearchNode root;    // final result, stores the number of moves

    // the search node class to store every state of the board
    private class SearchNode implements Comparable<SearchNode> {
        // to make sure the solution tree and the queue stores immutable objects
        ...
        public boolean isGoal(){
            return moves == priority; // manhattan == 0?
        }
    }
    ...
    public Solver(Board initial){
        ...
        while(!pq.isEmpty()){
            ...
            if (cur.isGoal()){
                // Determine if this came from the initial board
                SearchNode tmp = cur;
